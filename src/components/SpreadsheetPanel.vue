<template>
  <div class="header">
    ...
    <div v-if="spreadsheetUrl === null">
      <el-tooltip v-if="malformed" class="item" effect="dark" content="Input url is not Google Spreadsheet url." placement="top">
        <el-button disabled>Sync</el-button>
      </el-tooltip>
      <el-button v-else type="primary" @click="checkSync">Sync</el-button>
    </div>
    <div v-else>
      <el-button @click="checkSync" class="button-margin-right">Re-Sync</el-button>
      <el-popconfirm
...
<script lang="ts">
  ...
  export default class SpreadsheetPanel extends Vue {
    ...
    extractSpreadsheetId(url: string): string | null {
      const result = url.match(/^https:\/\/docs.google.com\/spreadsheets\/d\/([^/]+)\/.*$/);
